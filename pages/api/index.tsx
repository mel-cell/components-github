...
export default async function handler(request: Request) {
  try {
    ...
    // 1.5 Fetch Custom Image (if provided)
    let customImage = "";
    if (imageParam) {
      try {
        const imgRes = await fetch(imageParam);
        if (imgRes.ok) {
          const imgBuffer = await imgRes.arrayBuffer();
          const base64Img = Buffer.from(imgBuffer).toString("base64");
          const contentType = imgRes.headers.get("content-type") || "image/png";
          customImage = `data:${contentType};base64,${base64Img}`;
        }
      } catch (e) {
        console.error("Failed to fetch custom image:", e);
      }
    }

    // 2. Render to SVG
    const response = new ImageResponse(
      <BentoGrid data={data} customImage={customImage} />,
      {
        width: 1000,
        height: 850,
        fonts: [
